...
import type { VoxelChunkData, MapTileResponse, SurfaceColumnResponse, CaveConfig } from '@worldify/shared';

type InitMessage = { type: 'init'; seed: number; caveConfig?: CaveConfig };
...
self.onmessage = (e: MessageEvent<TerrainWorkerRequest>) => {
  ...
  if (msg.type === 'init') {
    source = new LocalTerrainSource(msg.seed, msg.caveConfig);
    return;
  }
  if (!source) return;
