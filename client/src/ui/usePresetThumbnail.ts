...
export function usePresetThumbnail(
  ...
  const partsKey = (parts ?? [])
    ...
    .join(';');
  const depKey = config
    ? `${config.mode}|${config.shape}|${config.size.x},${config.size.y},${config.size.z}|${config.material}|${config.thickness ?? 0}|${config.closed ?? 1}|${config.arcSweep ?? 0}|${rotKey}|${partsKey}|${textureState}`
    : '';
